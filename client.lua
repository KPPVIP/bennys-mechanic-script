...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			...
			if distance < 2 then
				drawTxt("Mekanik Menüsünü aç [E] ",4,0.5,0.93,0.50,255,255,255,180)
...
RegisterNUICallback("callbacks",function(data)
	if data then
		if data.type == "reparar" then
			...
			myveh.damage = 0.0
			stopAnim(false)
			TriggerEvent("Notify","sucesso","Araç onarımı <b>Başarılı</b>.",7000)
...
function isVehicleTooFar(veh)
	Citizen.CreateThread(function()
		while nui do
			...
			if distance > 7 then
				CCRIPT.closeNui()
				TriggerEvent("Notify","aviso","Araçtan çok uzaklaştınız.",7000)
			end
			Citizen.Wait(500)
		end
	end)
